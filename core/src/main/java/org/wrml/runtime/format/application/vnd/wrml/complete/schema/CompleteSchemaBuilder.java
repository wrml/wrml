...
public class CompleteSchemaBuilder {

    public CompleteSchemaBuilder() {
        ...
        if (declaredBaseSchemaUris != null && !declaredBaseSchemaUris.isEmpty()) {
            ...
            for (final URI baseSchemaUri : declaredBaseSchemaUris) {
                if (!addedBaseSchemaUris.contains(baseSchemaUri)) {
                    final ObjectNode baseSchemaNode = buildEmbeddedSchema(objectMapper, baseSchemaUri, schemaLoader, addedBaseSchemaUris);
    ...
    public String getTitleSlotName(final URI schemaUri, final SchemaLoader schemaLoader) {
        //
    ...
    private ObjectNode createSlot(final ObjectMapper objectMapper, final Prototype prototype, final String slotName) {
        ...
        switch (valueType) {
            ...
            case List: {
                ...
                if (elementValueType == ValueType.Model) {
                    ...
                    if (elementSchemaUri != null) {
                        final ObjectNode schemaNode = buildEmbeddedSchema(objectMapper, elementSchemaUri, schemaLoader, null);
            ...
            case Model: {
                final ObjectNode schemaNode = buildEmbeddedSchema(objectMapper, ((PropertyProtoSlot) protoSlot).getModelSchemaUri(), schemaLoader, null);
    ...
    private ObjectNode buildChoicesNode(final ObjectMapper objectMapper, final Type heapValueType, final SchemaLoader schemaLoader) {

        if (heapValueType == null || !(heapValueType instanceof Class<?>)) {
            return null;
        }

        final Class<Enum<?>> choicesEnumClass = (Class<Enum<?>>) heapValueType;
        ...
        final Enum<?>[] enumChoices  = choicesEnumClass.getEnumConstants();
        if (enumChoices != null && enumChoices.length > 0) {

            final ObjectNode valueMapNode = objectMapper.createObjectNode();
            choicesNode.put(PropertyName.valueMap.name(), valueMapNode);

            for (final Enum<?> enumChoice : enumChoices) {
                final String choice = String.valueOf(enumChoice);
                valueMapNode.put(enumChoice.name(), choice);
            }
        }
